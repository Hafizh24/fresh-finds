...
export default function DiscountTable({ handleEdit, handleHistory, handleDelete, discountData, currentPage, handlePageChange, totalPages, handleSortBy }) {
          ...
          <table className="mt-4 w-full min-w-max table-auto text-left">
              ...
              <tr>
                {TABLE_HEAD.map((head, index) => (
                  <th
                    ...
                    <Typography variant="small" color="blue-gray" className="flex items-center justify-between font-bold leading-none opacity-70">
                      {head}
            ...
            <tbody>
              {discountData?.map((item, index) => {
                const isLast = index === discountData.length - 1
                  ...
                  <tr key={index}>
                    ...
                    <td className={classes}>
                      <div className="flex flex-row gap-2">
                        <Typography variant="small" color="blue-gray" className="font-normal">
                          {item.ProductBranch?.Product?.name.length > 30 ? `${item.ProductBranch?.Product?.name.substring(0, 30)}...` : item.ProductBranch?.Product?.name}
                        </Typography>
                        <Tooltip
                          content={
                            <div className="w-80">
                              ...
                              <Typography variant="small" color="white" className="font-normal opacity-80">
                                <div className="flex flex-col">
                                  <Typography variant="small">Original Price: Rp.{item.ProductBranch?.Product?.price}</Typography>
                                  <Typography variant="small">Branch: {item.ProductBranch?.Branch?.name}</Typography>
                                </div>
                              </Typography>
                            </div>
                          }
                    ...
                    <td className={classes}>
                      <div className="flex flex-col">
                        <Typography variant="small" color="blue-gray" className="font-normal">
                          <span style={{ fontWeight: 'bold' }}>{item.min_purchase_amount}</span>
                        </Typography>
                      </div>
                    </td>
                    <td className={classes}>
