...
export const ModalBranchDelete = ({ modalDeleteOpen, setModalDeleteOpen, branchDetailData }) => {
  ...
  const handleDelete = async () => {
    try {
      ...
      const response = await axios.patch(`branches/delete/${branchDetailData.id}`, null, {
        headers: {
    ...
    <>
      <Dialog size="sm" open={modalDeleteOpen} handler={() => setModalDeleteOpen(!modalDeleteOpen)} className="px-3 pb-3 pt-5 flex flex-col items-center">
        ...
        <DialogBody className="flex flex-col gap-1 items-center">
          ...
          <h3 className="mb-1 text-[21px] md:text-[24px] text-[#28302A] font-bold text-center">Delete Branch</h3>
